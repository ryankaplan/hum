import { NOTE_NAMES, type ChordQuality, type ChordSymbol, type NoteName } from "./types";
...
const VALID_ROOTS = new Set<string>(NOTE_NAMES);
...
export function chordPitchClassNames(symbol: ChordSymbol): string {
  const [r, t, f] = chordSemitones(symbol.root, symbol.quality);
...
export function formatChordSymbol(symbol: ChordSymbol): string {
  const suffix = chordQualitySuffix(symbol.quality);
  const bass = symbol.bass == null ? "" : `/${symbol.bass}`;
  return `${symbol.root}${suffix}${bass}`;
}

export function durationSuffixToBeats(
  suffix: string,
  beatsPerBar: number,
): number | null {
  const divisor = DURATION_DIVISOR_BY_SUFFIX[suffix];
  return divisor == null ? null : beatsPerBar / divisor;
}
