...
export function TracksEditorPanel(props: TracksEditorPanelProps) {
  ...
  }, [timelineViewportWidth, view.timelineEndSec]);
    ...
    <Box overflow="hidden" {...dsPanel}>
        ...
        >
          ...
          >
                ...
                >
                  ...
                  {lane.segments.map((segment) => {
                    ...
                    const relativeSourceStart = Math.max(
                      0,
                      segment.sourceStartSec - lane.sourceStartSec,
                    );
                    const samples = samplePeaksForSegment(
                      lane.peaks,
                      lane.sourceDurationSec,
                      relativeSourceStart,
                      segment.durationSec,
                      bars,
                    );
                    ...
                    const brushWidthPercent =
                      activeBrush == null || segment.durationSec <= 0
                        ? 0
                        : ((activeBrush.endSec - activeBrush.startSec) / segment.durationSec) *
                          100;
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <polyline className="segment-volume-line" points={volumeLine} />
                        </svg>
                      </Box>
                    );
                  })}
            ...
            <Box
              ...
              left={`${view.playheadSec * TIMELINE_PX_PER_SEC}px`}
            />
          </Box>
        </Box>
      </Flex>
      ...
      >
        <input
          ...
          min={0}
          max={Math.max(1, Math.round(view.timelineEndSec * 1000))}
          step={1}
          value={Math.round(view.playheadSec * 1000)}
          onChange={(e) =>
            onCommand({ type: "seek", valueSec: parseInt(e.target.value, 10) / 1000 })
          }
          disabled={
