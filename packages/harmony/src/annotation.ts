...
import type { ChordQuality, ChordSymbol, MidiNote } from "./types";

export function chordToneFormula(chord: ChordSymbol): string {
...
export function labelHarmonyNoteForChord(
  chord: ChordSymbol,
  midi: MidiNote,
): string {
  const rootPitchClass = normalizePitchClass(rootSemitone(chord.root));
...
export function recipeIntervalPreferences(
  quality: ChordQuality,
...
function formulaIntervals(quality: ChordQuality): number[] {
  switch (quality) {
    case "major":
      return [0, 4, 7];
    case "minor":
